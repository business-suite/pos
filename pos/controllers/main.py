...
class PosController(http.Controller):
    ...
    def pos_web(self, config_id=False, **k):
        ...
        if not pos_session:
            return request.redirect('/web#action=pos.action_client_pos_menu')
        ...
        response = request.render('pos.index', context)
    ...
    def test_suite(self, mod=None, **kwargs):
        ...
        return request.render('pos.qunit_suite', qcontext=context)

    @http.route('/pos/sale_details_report', type='http', auth='user')
    def print_sale_details(self, date_start=False, date_stop=False, **kw):
        r = request.env['report.pos.report_saledetails']
        pdf, _ = request.env.ref('pos.sale_details_report').with_context(date_start=date_start, date_stop=date_stop)._render_qweb_pdf(r)
        pdfhttpheaders = [('Content-Type', 'application/pdf'), ('Content-Length', len(pdf))]
        return request.make_response(pdf, headers=pdfhttpheaders)

    @http.route('/pos/load_onboarding_data', type='json', auth='user')
    def load_onboarding_data(self):
        convert.convert_file(request.env.cr, 'pos', 'data/pos_onboarding.xml', None, mode='init', kind='data')
