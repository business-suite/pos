...
class PosConfig(models.Model):
    ...
    def _get_group_pos_manager(self):
        return self.env.ref('pos.group_pos_manager')

    def _get_group_pos_user(self):
        return self.env.ref('pos.group_pos_user')

    name = fields.Char(string='Name', index=True, required=True, help="An internal identification of the point of sale.")
    ...
    def _onchange_tipproduct(self):
        if self.iface_tipproduct:
            self.tip_product_id = self.env.ref('pos.product_product_tip', False)
        else:
    ...
    def _force_http(self):
        enforce_https = self.env['ir.config_parameter'].sudo().get_param('pos.enforce_https')
