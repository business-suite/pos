odoo.define('pos.Chrome', function(require) {
    'use strict';
    ...
    const PopupControllerMixin = require('pos.PopupControllerMixin');
    const Registries = require('pos.Registries');
    const IndependentToOrderScreen = require('pos.IndependentToOrderScreen');
    const contexts = require('pos.PosContext');
    const { identifyError, posbus } = require('pos.utils');
    ...
    class Chrome extends PopupControllerMixin(PosComponent) {
        ...
        async _closePos() {
            ...
            if (!this.env.pos || this.env.pos.db.get_orders().length === 0) {
                window.location = '/web#action=pos.action_client_pos_menu';
            }
        ...
        _onPlaySound({ detail: name }) {
            let src;
            if (name === 'error') {
                src = "/pos/static/src/sounds/error.wav";
            } else if (name === 'bell') {
                src = "/pos/static/src/sounds/bell.wav";
            }
        ...
        _preloadImages() {
            ...
            for (let imageName of staticImages) {
                const image = new Image();
                image.src = `/pos/static/src/img/${imageName}`;
            }
        }
        ...
        _buildChrome() {
            if ($.browser.chrome) {
                ...
                if (parseInt(chrome_version, 10) >= 50) {
                    loadCSS('/pos/static/src/css/chrome50.css');
                }
            }
